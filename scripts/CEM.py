...
history, scores = model(x_train, y_train, x_test, y_test, x_val, y_val,week_type, feature_types, course, num_epochs=15, n_weeks=num_weeks, n_features=num_features)
...
np.save('uniform_eq_results/CEM/'+course+'/changes_pn', np.array(changes).reshape(len(num_instances),-1))


# In[54]:


pns = np.array([explanation[i].PN for i in range(len(explanation))]).reshape(len(num_instances),-1)
...
diffs = pd.DataFrame(np.array(changes).reshape(len(num_instances),-1), columns=feature_names)
